...
<template>
  <div class="sm:text-center lg:mx-0 lg:text-left">
    <div v-if="!isFinished || error">
      <form class="mt-3 sm:flex" @submit="submit">
        ...
        <button
          :disabled="isFetching"
          type="submit"
          class="mt-3 w-full cursor-pointer rounded-md border border-transparent bg-accent-dark px-6 py-3 text-base font-semibold text-stone-900 shadow-lg transition-[background-color,box-shadow] duration-200 hover:bg-accent hover:shadow-xl focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-primary-dark focus:outline-hidden disabled:cursor-not-allowed disabled:bg-accent-dark sm:mt-0 sm:ml-3 sm:inline-flex sm:w-auto sm:shrink-0 sm:items-center">
          <span>
    ...
    <div v-if="isFinished && !error">
      <p class="text-base font-medium text-white">Thank you for signing up. Please check your email for a confirmation email.</p>
    </div>
