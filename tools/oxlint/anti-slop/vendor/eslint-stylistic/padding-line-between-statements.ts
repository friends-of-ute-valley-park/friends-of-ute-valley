...
function getActualLastToken(node: ASTNode, sourceCode: SourceCode): Token | null {
  ...
  const isSemicolonLessStyle =
    prevToken && nextToken && prevToken.range[0] >= node.range[0] && isSemicolonToken(semiToken) && !isTokenOnSameLine(prevToken, semiToken) && isTokenOnSameLine(semiToken, nextToken);
 ...
 * @private
 */
function verifyForAlways(context: RuleContext, prevNode: ASTNode, nextNode: ASTNode, paddingLines: [Token, Token][]): void {
  if (paddingLines.length > 0) return;
...
const StatementTypes: Record<string, NodeTestObject> = {
  ...
  'cjs-import': {
    test: (node, sourceCode) =>
      node.type === 'VariableDeclaration' && node.declarations.length > 0 && node.declarations[0]?.init != null && CJS_IMPORT.test(sourceCode.getText(node.declarations[0].init)),
  },
    ...
    create(context) {
      ...
      function match(node: ASTNode, type: StatementOption): boolean {
        ...
        if (isSelectorOption(type)) {
          ...
          if (statementType === undefined) throw new Error(`Padding rule invariant: unsupported statement type ${type}`);
      ...
      function verify(node: ASTNode): void {
        if (!node.parent || !['BlockStatement', 'Program', 'StaticBlock', 'SwitchCase', 'SwitchStatement', 'TSInterfaceBody', 'TSModuleBlock', 'TSTypeLiteral'].includes(node.parent.type)) {
          return;
        }
