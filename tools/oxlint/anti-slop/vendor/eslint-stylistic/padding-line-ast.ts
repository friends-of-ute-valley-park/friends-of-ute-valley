...
export const isTokenOnSameLine = (left: { loc: Location }, right: { loc: Location }): boolean => left.loc.end.line === right.loc.start.line;

/** Recognize declarations and expressions used by the upstream IIFE matcher. */
export const isFunction = (node: ESTree.Node): boolean => node.type === 'FunctionDeclaration' || node.type === 'FunctionExpression' || node.type === 'ArrowFunctionExpression';
...
export const skipChainExpression = (node: ESTree.Node): ESTree.Node => (node.type === 'ChainExpression' ? node.expression : node);
...
export const isTopLevelExpressionStatement = (node: ESTree.Node): node is ESTree.ExpressionStatement =>
  node.type === 'ExpressionStatement' && (node.parent.type === 'Program' || (node.parent.type === 'BlockStatement' && isFunction(node.parent.parent)));
