...
export const isStringLiteral = (node: ESTree.Node | null | undefined): node is ESTree.StringLiteral => node?.type === 'Literal' && typeof node.value === 'string';

export const isTagMember = (node: ESTree.Node | null | undefined): node is ESTree.MemberExpression =>
  node?.type === 'MemberExpression' &&
  ((!node.computed && node.property.type === 'Identifier' && node.property.name === '_tag') || (node.computed && isStringLiteral(node.property) && node.property.value === '_tag'));
