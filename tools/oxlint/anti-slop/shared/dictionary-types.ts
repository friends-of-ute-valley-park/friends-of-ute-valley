...
import { createTypeAliasEnvironment, hasVisibleTypeBinding, visibleTypeAlias, type TypeAliasEnvironment as LexicalTypeAliasEnvironment } from './type-alias-resolution.ts';
...
export function createTypeEnvironment(program: ESTree.Program, visitorKeys: Readonly<Record<string, readonly string[]>>): TypeEnvironment {
...
function isEffectivelyEmptyMember(member: ESTree.TSSignature): boolean {
  return (
    member.type === 'TSPropertySignature' && member.optional === true && member.typeAnnotation !== null && member.typeAnnotation !== undefined && isNeverType(member.typeAnnotation.typeAnnotation)
  );
}
...
function unsafeDirectValue(type: ESTree.TSType, environment: TypeEnvironment, substitutions: TypeAliasEnvironment, resolvingAliases: ReadonlySet<string>): UnsafeDictionary['unsafeValue'] | null {
  const unwrapped = unwrapTransparentType(type);
...
function dictionaryValueTypes(type: ESTree.TSType, environment: TypeEnvironment, substitutions: TypeAliasEnvironment, resolvingAliases: ReadonlySet<string>): readonly ResolvedType[] {
  const unwrapped = unwrapTransparentType(type);
...
function hasBroadRecordKey(type: ESTree.TSTypeReference, environment: TypeEnvironment, substitutions: TypeAliasEnvironment): boolean {
  const key = type.typeArguments?.params[0];
  return key === undefined || isBroadMappedKey(key, environment, substitutions);
}

function isBroadMappedKey(type: ESTree.TSType, environment: TypeEnvironment, substitutions: TypeAliasEnvironment, visitedAliases: ReadonlySet<string> = new Set()): boolean {
  const unwrapped = unwrapTransparentType(type);
...
function classifyAliasBroadTarget(type: ESTree.TSType, environment: TypeEnvironment, substitutions: TypeAliasEnvironment, resolvingAliases: ReadonlySet<string>): WideningTarget | null {
  const unwrapped = unwrapTransparentType(type);
  if (unwrapped.type === 'TSUnknownKeyword') return { kind: 'unknown' };
  if (unwrapped.type === 'TSObjectKeyword') return { kind: 'object' };
  if (unwrapped.type === 'TSTypeLiteral') {
