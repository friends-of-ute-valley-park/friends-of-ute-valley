...
export function isKnownArrayExpression(sourceCode: SourceCode, node: ESTree.Node, visited = new Set<Variable>()): boolean {
  ...
  if (node.type === 'CallExpression') {
    ...
    return (
      method !== null && ['map', 'filter', 'flatMap', 'slice', 'concat', 'toSorted', 'toReversed', 'toSpliced'].includes(method.name) && isKnownArrayExpression(sourceCode, method.object, visited)
    );
  }
